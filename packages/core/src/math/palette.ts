...
export interface RichColor extends Color {
    alpha(a: number): RichColor;
}

export interface ColorScale extends RichColor {
    readonly 50: RichColor;
    readonly 100: RichColor;
    readonly 200: RichColor;
    readonly 300: RichColor;
    readonly 400: RichColor;
    readonly 500: RichColor;
    readonly 600: RichColor;
    readonly 700: RichColor;
    readonly 800: RichColor;
    readonly 900: RichColor;
    readonly 950: RichColor;
}

function richColor(c: Color): RichColor {
    return {
        r: c.r,
        g: c.g,
        b: c.b,
        a: c.a,
        alpha(a: number): RichColor {
            return richColor({ r: c.r, g: c.g, b: c.b, a });
        },
    };
}

type ShadeMap = Record<Shade, string>;

function colorScale(shades: ShadeMap): ColorScale {
    const base = richColor(fromHex(shades[500]));
    const scale = base as ColorScale;

    for (const key of Object.keys(shades)) {
        const shade = Number(key) as Shade;
        (scale as unknown as Record<number, RichColor>)[shade] = richColor(fromHex(shades[shade]));
    }
...
// Standalone rich colors (not scales)
export const white = richColor({ r: 1, g: 1, b: 1, a: 1 });
export const black = richColor({ r: 0, g: 0, b: 0, a: 1 });
